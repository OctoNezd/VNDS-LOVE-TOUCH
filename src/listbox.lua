...
local function create_listbox(self)
    ...
    self.onclose = self.onclose or function()
    end
    self.media = self.media or -listbox_pad
    ...
    input_event = on("input", function(input)
        ...
        if input == "up" or input == "down" then
            if chosen.onchange then
                chosen.onchange(chosen)
            end
        end
        if input == "a" then
            local outcome = chosen.action(chosen, close)
            if self.closable and outcome then
                close()
            end
            if not self.closable then
                close()
            end
    ...
    draw_event = on("draw_choice", function()
        ...
        local w = 3 * listbox_pad + max_w + self.media
        local h, y_selected = listbox_pad, 0
        for i, c in ipairs(self.choices) do
            h = h + font_height(c.text) + listbox_pad
        ...
        local text_y = y + listbox_pad
        for i, c in ipairs(self.choices) do
            lg.setColor(1, 1, 1)
            if c.media then
                c.media(x + listbox_pad, text_y)
            end
            if i == self.selected then
                lg.setColor(.506, .631, .757)
            end
            lg.print(c.text, x + 2 * listbox_pad + self.media, text_y)
            text_y = text_y + listbox_pad + font_height(c.text)
        end
        lg.setFont(font)
        return false
    end)
